...
\begin{frame}{Cos'è la logica (1)}
	La parola \alert{logica} deriva dal greco \textit{logos}, che vuol dire ``parola'', ``pensiero'', ''ragionamento''.

	Una prima definizione di logica potrebbe essere:
	\begin{definition}[Logica]
		La logica è lo studio dei ragionamenti.
	\end{definition}
	\pause
	\begin{example}[Esempi di ragionamenti]
		\begin{columns}
			\column{0.5\textwidth}
			\centering
...
\begin{frame}{Cos'è la logica (2)}
	Il termine \alert{ragionamento} è però ambiguo, perché indica sia un \emph{processo} che un \emph{prodotto}.
	\begin{itemize}
		\item il \emph{processo} del ragionamento è il modo in cui si giunge a una conclusione, ed è di pertinenza più dello psicologo o del neurobiologo che del logico;
		\item il \emph{prodotto} del ragionamento è invece una discorso, come quelli visti nella diapositiva precedente; è chiamato anche \alert{argomentazione} o \alert{argomento}.
	\end{itemize}
	Le seguenti definizioni di logica sono quindi preferibili:

	\begin{block}{Definizioni (Logica)}
		La logica è:
		\begin{itemize}
			\item  lo studio dei metodi e dei principi usati per distinguere un ragionamento corretto da uno scorretto.\footnote{Irving M. Copi, Carl Cohen. Introduzione alla logica. Mulino}
			\item lo studio delle argomentazioni.\footnote{Achielle Varzi, John Nolt, Dennis Rohatyn. Logica, 3° edizione. McGraw Hill}
		\end{itemize}
	\end{block}
\end{frame}

\begin{frame}{Ragionamenti e inferenze}
	Noi useremo i termini ragionamento, argomentazione e argomento come sinonimi.

	Vediamo altri esempi di ragionamenti:
	\begin{itemize}
		\item Tutti gli uomini sono mortali. Poichè Socrate è un uomo, ne segue che Socrate è mortale.
		\item omissis di argomento induttivo
		\item Tutti i numeri razionali sono esprimibili come frazioni di interi. Pi greco non è esprimibile come frazione di interi, quindi non è un numero razionale. Tuttavia pi greco è un numero. Ne segue che esiste almeno un numero irrazionale.
	\end{itemize}

	Noi restringeremo il nostro campo di studio a ragionamenti di un forma particolare: si tratta dei ragionamenti \alert{deduttivi} \alert{semplici} che chiameremo \alert{inferenze}.
\end{frame}

\begin{frame}{Inferenze}
	%Ci occuperemo in particolare di forme di ragionamento molto specifiche che chiamiamo \alert{inferenze}.
...
\begin{frame}{Proposizioni}
	...
	\begin{example}[Proposizioni \checkmark o no? \xmark]
		\begin{itemize}
			\item Elisa ama Massimo \pause \checkmark
			\item Elisa ama Massimo? \pause \xmark\ (è una \emph{domanda})
			\item $4 + 2 = 6$ \pause  \checkmark
			\item $4 + 2 = 5$ \pause \checkmark (è falsa, ma è comunque una proposizione)
			\item $3 + 5^2$ \pause \ \xmark \ (è una \emph{descrizione})
			\item Tutti gli uomini sono mortali \pause \checkmark
			\item Lazzaro, alzati e cammina! \pause \xmark \ (è un \emph{ordine})
			\item Quel ramo del lago di Como che volge a mezzogiorno \pause \xmark \ (è una \emph{descrizione})
		\end{itemize}
	\end{example}
\end{frame}
...
\begin{frame}{Valori di verità}
	% Se una proposizione è vera, diciamo anche che assume il \alert{valore di verità} vero (\alert{V}). Analogamente, se una proposizione è falsa, diciamo anche che assume il \alert{valore di verità} falso (\alert{F}).
...
%
%	\medskip
%	\textbf{Indicatore di conclusiome}\\
...
% 	\end{example}
% 	\begin{block}{Nota}
% 		In alternativa, possiamo dire che $C$ è \alert{conseguenza logica} delle proposizioni $P_1, \ldots, P_n$ qualore è impossibile che $P_1, \ldots, P_n$ siano tutte vere e nello stesso tempo $C$ sia falsa.
% 	\end{block}
% \end{frame}
...
\begin{frame}{Inferenze corrette}
	...
	\begin{example}[Inferenza corretta]
		...
		% 	Dunque, Carlo è ligure
		% \end{inference}

		% \medskip
		% $\Big\Downarrow$\rlap{ che in futuro scriveremo come}
...
\begin{frame}{Inferenze e contesto}
	Non si considerano corrette inferenze che dipendono dalla conoscenza del contesto.
...
% \begin{frame}{Inferenze e struttura sintattica (2)}
% 	Un esempio ancora più complesso:
% 	\begin{example}
% 		\begin{inference}
% 			O due buchi neri si sono scontrati,\hspace{6cm}\hfill\\ \hfill oppure il rivelatore di onde gravitazionali è rotto.\\
% 			Il rivelatore di onde gravitazionali non è rotto.\\
% 			\hline
% 			Due buchi neri si sono scontrati.
% 		\end{inference}
...
% 	domandarvi perplessi cosa siano le onde gravitazionali, ma non avreste difficoltà
% 	a seguire la logica del suo ragionamento.

% 	\pause
% 	\begin{block}{Osservazione}
% 		La correttezza di una inferenza non dipende dalla verità delle proposizioni che la compongono, ma dalla sua ``forma logica''.
% 	\end{block}
% \end{frame}
...
\begin{frame}{Inferenze corrette e verità delle conclusioni}
	\medskip
	Bisogna distinguere la correttezza della inferenze dalla verità della conclusione!
...
\note{Esercizi Intro.1, Intro.4, Intro.7}

\section{Logica dei predicati}

% \begin{frame}{Regole di inferenza a livello predicativo}
...
\begin{frame}{Ragionamento induttivo e abduttivo}
	Esistono altre forme di ``ragionamento'', oltre a quello deduttivo visto finora.
	...
	\uncover<4->{
	\begin{block}{Nota}
		Dal punto di visto deduttivo, queste inferenze non sono corrette. L'ultima, in particolare, è propria quella che abbiamo chiamato ``fallacia dell'affermazione del conseguente''.
	\end{block}
	}
...
% 	Tutti i tipi di ragionamento visti pocanzi \alert{non sono monot\`oni}: nuove premesse possono trasformare la conclusione del ragionamento.
% 	\begin{center}
% 		\begin{inference}
% 			Normalmente i polacchi non sanno parlare italiano\\
% 			Karol è polacco\\
...
% 	Invece le inferenze (e i ragionamenti deduttivi) \alert{sono monot\`one}. Se aggiungo nuove premesse, l'inferenza rimane corretta.
...
\begin{frame}{Inferenze e monot\`onia}
	\begin{itemize}
		...
		\begin{itemize}
			...
			\end{itemize}
			\item se applicano inferenze corrette a premesse corrette per ottenere conclusioni corrette (teoremi)
