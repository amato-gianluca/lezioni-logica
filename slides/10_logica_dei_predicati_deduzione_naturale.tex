...
\begin{frame}{Introduzione del quantificatore universale}
    ...
    \begin{center}
            \framebox{\begin{minipage}{0.8\textwidth}
                ...
                \end{prooftree}
                \begin{center}
                    se $t$ non appare in nessuna ipotesi non cancellata
                \end{center}
            \end{minipage}}
    \end{center}

    \begin{example}[$\forall x (Px \to Qx), \forall x Px \to \forall x Qx$]
        \begin{prooftree}
            \AXC{$\forall x (Px \to Qx)$}
            \RightLabel{($\elim\forall$)}
...
\begin{frame}{Eliminazione del quantificatore esistenziale}
    ...
    \begin{center}
        \framebox{\begin{minipage}{0.8\textwidth}
            ...
            \end{prooftree}
            \begin{center}
                se $t$ non compare in $\psi$ e in nessuna ipotesi non cancellata di $\psi$
            \end{center}
        \end{minipage}}
    \end{center}
...
\begin{frame}{Teoremi di correttezza e completezza}
    Ovviamente, anche per la logica dei predicati valgono i teoremi di correttezzae e completezza forte.
