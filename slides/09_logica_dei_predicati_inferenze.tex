...
\begin{frame}{Formule valide (2)}
    ...
    \begin{example}
        ...
        \begin{itemize}
            \item $(\forall x Px) \vee \neg (\forall x Px)$ --- istanza di $A \vee \neg A$
            \item $(\forall x \exists y Qxy) \to (\forall x \exists y Qxy)$ --- istanza di $A \to A$
            \item  $(\forall x Px) \vee Qa \iff Qa \vee (\forall x Px)$ --- istanza di $A \iff A$
        \end{itemize}
    \end{example}
\end{frame}
...
\begin{frame}{Equivalenze derivate dalla logica proposizionale}
    ...
    \begin{example}
        ...
        \]
        tramite la sostituzione $A \mapsto \forall x Px$ e  $B \mapsto \exists x(Qx \wedge \forall y Rxy)$.
    \end{example}
    ...
    \[
        \phi_1 \wedge \phi_2 \equiv \phi_2 \wedge \phi_1 \enspace.
    \]
\end{frame}
...
\begin{frame}{Negazione e quantificatori (1)}
    ...
    \]
    \begin{example}
        Supponiamo che l'univero del discorso siano gli esseri umani. Allora, se $\phi$ è la funzione proposizionale ``\prop{$x$ sa volare}\/'':
        \begin{itemize}
            \item $\neg \exists x \phi$ è ``\prop{non esiste un essere umano che sa volare}\/''
            \item $\forall x \neg \phi$ è ``\prop{tutti gli essere umani non sanno volare}\/''
        \end{itemize}
...
\begin{frame}{Quantificatore universale e disgiunzione (2)}
    ...
    \[
        (\forall x Rx) \vee (\forall x Gx) \models \forall x (Rx \vee Gx)
    \]
    ...
    \[
        (\forall x \phi_1) \vee (\forall x \phi_2) \models \forall x (\phi_1 \vee \phi_2)
    \]
\end{frame}
...
\begin{frame}{Quantificatore esistenziale e congiunzione}
    ...
    \[
        \exists x (Rx \vee Gx) \equiv (\exists x Rx) \vee (\exists x Gx)
    \]
    ovvero
    \begin{proposition}
