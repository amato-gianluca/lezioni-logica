...
\begin{frame}{Termini aperti (2)}
	...
	\begin{example}
		...
		$\Downarrow$\\
		Esiste x tale l'autore di x è nato nel 1920.
	\end{example}
\end{frame}
...
\begin{frame}{Il quantificatore universale (2)}
	...
	\begin{itemize}
		\item $x \mapsto a$: \prop{se la lettere ``a'' è una vocale allora ``a'' compare nella parola ``aiuole''}
		\item $x \mapsto b$: \prop{se la lettere ``b'' è una vocale allora ``b'' compare nella parola ``aiuole''}
		\item $x \mapsto c$: \prop{se la lettere ``c'' è una vocale allora ``c'' compare nella parola ``aiuole''}
		\item \ldots
		\item $x \mapsto l$: \prop{se la lettere ``l'' è una vocale allora ``l'' compare nella parola ``aiuole''}
		\item \ldots
		\item $x \mapsto z$: \prop{se la lettere ``z'' è una vocale allora ``z'' compare nella parola ``aiuole''}
	\end{itemize}
\end{frame}

\begin{frame}{Il quantificatore universale (3)}
	Si può verificare che queste proposizioni sono effettivamene tutte vere. Facciamo solo un paio di esempi:
	\begin{itemize}
		\item \prop{se la lettere ``a'' è una vocale allora ``a'' compare nella parola ``aiuole''}\\[0.1cm]
		      È vera perché sono vere sia l'antecedente che il conseguente dell'implicazione.\\[0.2cm]

		\item \prop{se la lettere ``b'' è una vocale allora ``b'' compare nella parola ``aiuole''}\\[0.1cm]
		      È vera perché l'antecedente dell'implicazione è vero.\\[0.2cm]

		\item \prop{se la lettere ``l'' è una vocale allora ``l'' compare nella parola ``aiuole''}\\[0.1cm]
...
\begin{frame}{Controesempio}
	Nella pratica, dovremo trovare altri modi di determinare se una proposizione è vera, per esempio scrivendo una \alert{dimostrazione}, ovvero una inferenza che partendo da proprietà note (assiomi) arriva ad inferire la proprietà voluta.
...
\begin{frame}{Conseguttura di Goldbach}
	Ma se non trovo nessun controesempio ? Ci sono tante proposizioni sui numeri che non sappiamo se sono vere o fale, come la \alert{congettura di Goldbach}.
	\begin{proposition}
...
\begin{frame}{Esempio di quantificazione universale su grafi}
	\begin{columns}
		...
		\begin{itemize}
			\item \prop{Tutti i nodi connesso con c sono verdi}
		\end{itemize}
		ovvero
		\begin{itemize}
		...
		\begin{itemize}
			...
			\item $x \mapsto e$: \prop{sse e è connesso con c allora e è verde}.
		\end{itemize}
	\end{columns}
\end{frame}
...
\begin{columns}
	...
	\end{proposition}
	è vera se esiste almeno un valore dell'universo del discorso che possiamo rimpiazzare in $x$ per rendere vera proposizione nei $\ldots$
...
\begin{frame}{Più quantificatori (1)}
	...
	\begin{itemize}
		...
		\item $x \mapsto a$: \prop{se b è verde esiste y tale che y è giallo e b è connesso con y}
		\item $x \mapsto a$: \prop{se c è verde esiste y tale che y è giallo e c è connesso con y}
		\item $x \mapsto a$: \prop{se d è verde esiste y tale che y è giallo e d è connesso con y}
		\item $x \mapsto a$: \prop{se e è verde esiste y tale che y è giallo ed e è connesso con y}
	\end{itemize}
	Verifchiamole una alla volta.
\end{frame}
...
\begin{frame}{Più quantificatori (3)}
	Otteniamo così che la sostituzione $x \mapsto d$ da origine ad un controesempio
	\begin{proposition}
