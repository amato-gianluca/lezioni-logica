...
\begin{frame}
    \bookpage{sui capitoli 1 (solo la prima pagina) e 2 (esclusa sezione 2.3)}
\end{frame}
...
\begin{frame}{Proposizioni semplici e composte}
    ...
    \medskip Studieremo quindi più in dettaglio la struttura di una proposizione.
...
\begin{frame}{Connettivi}
    \begin{columns}
        ...
        \column{0.3\textwidth}
        \begin{center}
            \includegraphics[width=3cm,keepaspectratio]{img/puzzle.png}
        \end{center}
    \end{columns}
    \begin{example}
        \propshape
...
\begin{frame}{Logica delle proposizioni}
    Le proposizioni semplici hanno una loro struttura interna:
    \begin{itemize}
        \item \prop{Carlo è ligure} afferma che un certo \emph{individuo} (Carlo) gode di una certa proprietà (essere ligure).
        \item \prop{Carlo è più alto di Mario} afferma che c'è una \emph{relazione} (essere più alto di) tra Carlo e Mario.
        \item \prop{Tutti gli uomini sono mortali} ha una struttura complessa e contiene il quantificatore \emph{tutti}.
    \end{itemize}

    \pause
    \medskip
    Nella prima parte del corso ci occuperemo della \alert{logica delle proposizioni}, detta anche \alert{logica proposizionale} o \alert{logica enunciativa}:
    \begin{itemize}
        \item studia la relazione che intercorre tra le proposizioni;
        \item ignora la struttura interna delle proposizioni semplici.
    \end{itemize}

    \pause
    \medskip
    Nella seconda parte del corso ci occuperemo della \alert{logica dei predicati}, che si occupa di indagare anche la struttura interna delle proposizioni semplici.
\end{frame}
...
\begin{frame}{Dettagli sulla negazione}
    ...
    \begin{itemize}
        \item \alert{$\neg$ (in logica)}
        \item \alert{not (in inglese, Python e C++)}
        \item $\texttt{!}$ (in C, C++, Java, ecc.)
...
\begin{frame}{Congiunzione nella lingua italiana (1)}
    ...
    \begin{example}
        \prop{Carlo \conn{e} Maria sono appassionati di baseball}\medskip

        \pause
        \hspace{2cm} è una abbreviazione di\medskip

        \prop{Carlo è appassionato di baseball \conn{e} Maria è appassionata di baseball}
    \end{example}

    \pause
    \begin{example}
        \prop{La partita è avvincente \conn{e} combattuta}\medskip

        \pause
        \hspace{2cm} è una abbreviazione di\medskip

        \prop{La partita è avvincente \conn{e} la partita è combattuta}
    \end{example}
\end{frame}
...
\begin{frame}{Congiunzione nella lingua italiana (2)}
    ...
    \begin{example}
        \prop{Carlo e Maria sono amici}\medskip

        \pause
        \hspace{2cm} \textbf{non è} una abbreviazione di\medskip

        \prop{Carlo è amico \conn{e} Maria è amica}\medskip
    \end{example}
    ...
    \begin{example}
        \prop{Roma è la capitale d'Italia \conn{ma} Parigi è la capitale della Francia}
    \end{example}
\end{frame}
...
\begin{frame}{Dettagli della congiunzione}
    ...
    \begin{itemize}
        \item \alert{$\land$ (in logica)}
        \item \alert{and (in inglese, Python e C++)}
        \item $\texttt{\&\&}$ (in C, C++, Java, ecc.)
...
\begin{frame}{Doppia lettura della tavola di verità}
    \begin{tikz}[overlay]
        \node at (12,-4.5) {        \begin{tabular}{c|c||c}
                $A$ & $B$ & $A \land B$ \\
                \hline
                F   & F   & F           \\
                F   & V   & F           \\
    ...
    \begin{itemize}
        \item da sinistra a destra: se sappiamo il valore di verità delle proposizioni di base, possiamo determinare il valore di verità della proposizione risultante.
              ...
              \medskip
              A è vera e B è falsa $\longrightarrow$ $A \land B$ è falsa.
              \pause

        \item da destra a sinistra: se sappiamo il valore di verità della proposizione composta, possiamo determinare quali sono le possibili combinazioni di valori di verità delle proposizioni di base.

              \medskip
              $A \land B$ è vera $\longrightarrow$ c'è un'unica possibilità: \pause
              \begin{itemize}
                  \item $A$ vera, $B$ vera
              \end{itemize}

              \medskip
              $A \land B$ è falsa $\longrightarrow$ ci sono tre possibilità: \pause
              \begin{itemize}
...
\begin{frame}{Esempio di connettivo non vero-funzionale}
    ...
    \begin{itemize}
        \item \prop{Lucia ha mangiato una mela \conn{mentre} ha guardato la televisione.}
    \end{itemize}
    ...
    \uncover<2->{
        ...
        \[
            A \equiv \text{\prop{Lucia ha mangiato una mela}} \qquad B \equiv \text{\prop{Lucia ha guardato la televisione}}
        \]
...
\begin{frame}{Disgiunzione nella lingua italiana}
    ...
    \begin{example}
        \prop{Roma è la capitale d'Italia \conn{o} Lione è la capitale della Francia}
    \end{example}
    \pause
    ...
    \begin{example}
        \prop{Roma è la capitale d'Italia \conn{o} Roma è la capitale della Francia}
    \end{example}
    Spesso, in questi casi, abbreviamo la frase, come già visto per la congiunzione
    \begin{example}
        \prop{Roma è la capitale d'Italia o della Francia}
    \end{example}
\end{frame}
...
\begin{frame}{Disgiunzione inclusiva ed esclusiva (1)}
    ...
    \begin{example}
        \prop{Roma è la capitale d'Italia \conn{o} Parigi è la capitale della Francia}
    \end{example}
...
\begin{frame}{Dettagli della disgiunzione inclusiva}
    Simboli usati per scrivere la disgiunzione inclusiva sono:
    \begin{itemize}
        \item \alert{$\lor$ (in logica)}
        \item \alert{or (in inglese, Python e C++)}
        \item $\texttt{||}$ (in C, C++, Java, ecc.)
    ...
    \uncover<2->{
        \bigskip
        Questa è la tavola di verità della disgiunzione inclusiva:
...
\begin{frame}{Dettagli della disgiunzione esclusiva}
    ...
    \begin{itemize}
        ...
        \item \alert{\texttt{!=} (in Python, Java, C, C++, ecc.)}
...
\begin{frame}{Disgiunzione inclusiva ed esclusiva nella lingua italiana (1)}
    ...
    \begin{example}
        ...
        \begin{itemize}
            \item \prop{La capitale della California è Sacramento \conn{o} Los Angeles}
        \end{itemize}
...
\begin{frame}{Disgiunzione inclusiva ed esclusiva nella lingua italiana (2)}
    \begin{example}
        Se sapete che
        \begin{itemize}
            \item \prop{La bandiera della California contiene una stella \conn{o} un orso}
        \end{itemize}
    ...
    Ad esempio:
    \begin{itemize}
        \item \prop{La bandiera della California contiene una stella \conn{o} un orso \conn{o entrambi}}.
    \end{itemize}
\end{frame}
...
\begin{frame}{Disgiunzione inclusiva ed esclusiva nella lingua italiana (3)}
    ...
    \begin{itemize}
        \item Se la disgiunzione è l'unico connettivo, essa è tipicamente esclusiva.
              ...
              \begin{itemize}
                  \item \prop{puoi comprare un gelato \conn{o} un pasticcino}
              \end{itemize}
        ...
        \item Se la disgiunzione appare nell'antecedente di una implicazione (vedremo dopo di cosa si tratta), è tipicamente inclusiva.
              ...
              \begin{itemize}
                  \item \prop{\conn{se} sono laureato in economia \conn{o} in informatica sono ammesso al concorso}
              \end{itemize}
              ovviamente si intende che sono ammesso anche se ho entrambe le lauree.
    \end{itemize}
\end{frame}
...
\begin{frame}{Domande dagli studenti}
    ...
    \medskip
    \emph{Perché uno dovrebbe affermare ``$2 + 2 = 4$ \conn{oppure} $5$ è dispari'' invece di ``$2 + 2 = 4$ \conn{e} $5$ è dispari''?}

    \medskip
    Ovviamente in questo caso usare ``\conn{oppure}'' può sembrare stupido, ma solo perché sappiamo a priori che entrambe le proposizioni di base sono vere. Se introduciamo delle variabili, e scriviamo una proposizione del tipo
    \begin{center}
    ...
    \smallskip
    Il fatto che la disgiunzione in matematica sia inclusiva vuol dire che per $x=2$ la proposizione di cui sopra è vera. Se la disgiunzione fosse interpretata in maniera esclusiva, per $x=2$ la proposizione sarebbe falsa.

    \pause\smallskip
    In realtà ``$x < 4$ \conn{oppure} $x$ è pari'' si dovrebbe più correttamente chiamare \emph{funzione proposizionale}, ma rimandiamo questo discorso a quando introdurremo la logica dei predicati.
\end{frame}

\section{Implicazione}

\begin{frame}{Implicazione}
    L'\alert{implicazione}, più correttamente chiamata \alert{implicazione materiale}, collega due proposizioni $A$ e $B$ tra di loro. La proposizione risultante è sempre vera, tranne quando la prima è vera e la seconda è falsa (ricorda la definizione di correttezza di una inferenza).
...
\begin{frame}{Interpretazione dell'implicazione (1)}
    ...
    \begin{example}
        ...
        \begin{itemize}
            \item  \prop{\conn{se} Carla è alla festa \conn{allora} anche Luca è alla festa}
        \end{itemize}
...
\begin{frame}{Interpretazione dell'implicazione (3)}
    Se avete dubbi sulla seconda riga della tabella di prima è perché spesso in italiano diciamo \conn{se \ldots allora} ma in realtà intendiamo \conn{se e solo se} (che è un altro connettivo).

    \pause
    \begin{example}
        Supponiamo che un amico ci ha assicurato che:
        \begin{itemize}
            \item \prop{\conn{se} piove \conn{allora} vengo a prenderti in macchina}
        \end{itemize}
        Quali di queste situazioni possono verificarsi (se l'amico mantiene la promessa, cioè se la proposizione di prima è vera)?
        \begin{center}
            \begin{tabular}{c|c||c}
...
\begin{frame}{Interpretazione dell'implicazione in italiano (2)}
    Se proviamo a interpretare la tavola di verità dell'implicazione ``da sinistra verso destra'', otteniamo spesso qualcosa di poco sensato.
    ...
    \medskip\pause
    Davvero direste nella vita di tutti i giorni che l'affermazione
    \begin{center}
        \prop{\conn{se} gli elefanti volano \conn{allora} 3+2=0}
    \end{center}
    è vera?
\end{frame}

\begin{frame}{Implicazione e rapporti di causa-effetto}
    Queste proposizioni sono abbastanza innaturali. Questo perché nel linguaggio naturale quasi sempre l'uso del \conn{se \ldots allora} suggerisce un rapporto di causa-effetto tra antecedente e conseguente.
    \begin{example}
        Quando diciamo\\
        \begin{center}
            \prop{\conn{se} Carla è alla festa \conn{allora} anche Luca è alla festa}
        \end{center}
        intendiamo che c'è qualche rapporto di causa-effetto tra le due cose.
    \end{example}

    \medskip
    L'implicazione che trattiamo noi:
    \begin{itemize}
        \item prescinde da ogni rapporto di causa-effetto ed analizza solo lo stato delle cose;
        \item per distinguerla da altri usi dell'implicazione, la chiamiamo appunto \alert{implicazione materiale}.
    \end{itemize}
\end{frame}
...
%         }
%         \uncover<4->{
%             Dunque, quando ``\conn{se} Carla è alla festa \conn{allora} anche Luca è alla festa'' è falso, l'unica possibilità è che ``Carla è alla festa'' e ``Luca non è alla festa''.
%         }
%     \end{example}
% \end{frame}

% \begin{frame}{Implicazione e quantificazione implicita}
%     In molti usi del \conn{se \ldots allora} nel linguaggio naturale c'è una quantificazione implicita.
...
%         \item pertanto, la proposizione è falsa, perché non è detto che se è nuvoloso deve per forza debba piovere;
...
\begin{frame}{Implicazione e linguaggio naturale}
    In conclusione,
    \begin{itemize}
        \item l'implicazione nel linguaggio naturale è spesso diversa dall'implicazione materiale in logica;
        ...
        \item l'implicazione materiale è molto usata in matematica, ma quasi sempre in presenza di variabili:
              \begin{itemize}
                  \item è difficile trovare affermazioni del tipo ``\prop{\conn{se} 8 è divisibile per 4 \conn{allora} 8 è divisibile per 2}\/'';
                  \item è molto più normale trovare ``\prop{\conn{se} $x$ è divisibile per 4 \conn{allora} $x$ è divisibile per 2}\/''.
              \end{itemize}
    \end{itemize}
    \pause
    Ad ogni modo,
    \begin{itemize}
        \item nello studio della logica, e quindi in tutto il resto del corso, tutte le volte in cui compare \conn{se \ldots allora} esso va interpretato come implicazione materiale;
...
\begin{frame}{Altre forme dell'implicazione in linguaggio naturale (2)}
    ...
    Alcune osservazioni:
    \begin{itemize}
        \item A \conn{se} B: è solo un modo contorto di scrivere ``se B allora A'', ovvero $B \to A$.
        \item A \conn{solo se} B: vuol dire che A si può verificare solo se si verifica anche B. Quindi, non appena sappiamo che A è vero, per forza deve essere vero anche B, ovvero $A \to B$.
        ...
        \item A \conn{è condizione necessaria per} B: vuol dire che perché si verifichi B deve necessariamente verificarsi A. Quindi, non appena sappiamo che B è vero, A deve essere pure vero. In sostanza, $B \to A$.
...
\begin{frame}{Doppia implicazione}
    ...
    \begin{itemize}
        ...
        \item \alert{\texttt{==} (in Python, C, C++, Java, ecc.)}
    \end{itemize}
...
\begin{frame}{Interpretazione della doppia implicazione in italiano (1)}
    Molti dei problemi di corrispondenza tra implicazione e linguaggio naturale continuano a valere per la doppia implicazione. Come sempre, l'interpretazione più semplice è con la lettura ``da destra a sinistra''.

    \pause
    \begin{example}
        Supponiamo che un amico ci abbia assicurato che:
        \begin{itemize}
            \item \prop{vengo a prenderti in macchina \conn{se e solo se} piove}
        \end{itemize}
        A parte il fatto che il vostro amico parla in maniera un po' strana\ldots quali di queste situazioni possono verificarsi (se l'amico mantiene la promessa, cioè se la proposizione di prima è vera)?
        \begin{center}
            \begin{tabular}{c|c||c}
...
\begin{frame}{Esercizi consigliati}
    Esercizi da 1 a 9 del Capitolo 2.
\end{frame}

\note{Esercizi 2.1, 2.2, 2.3, 2.4, 2.6, 2.7, 2.8}
