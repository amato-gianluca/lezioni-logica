...
\begin{frame}{Tautologie (2)}
    ...
    \begin{example}
        ``\textit{Se Carla è italiana e laureata, allora è italiana.}'' è sicuramente una proposizione vera, anche se non sappiamo chi sia Carla.
    \end{example}
...
\begin{frame}{Contraddizione}
    ...
    \begin{example}
        Carla è laureata ma non è laureata.
    \end{example}
\end{frame}
...
\begin{frame}{Tautologie e contraddizioni}
    ...
    \begin{block}{Osservazione}
        In futuro, useremo le lettere terminali dell'alfabeto come $X$, $Y$, $Z$ per indicare una forma proposizionale, mentre useremo le lettere iniziali per le lettere proposizionali.
    \end{block}
\end{frame}
...
\begin{frame}{Tautologie notevoli: principio di non contraddizione}
    ...
    \begin{example}[in italiano]
        ``\textit{Carla è laureata e non è laureata }'' è sicuramente falsa, anche se non sappiamo chi è Carla. Pertanto la sua negazione .``\textit{Non è vero che Carla è laureata e non è laureata}'' è sicuramente vera.
    \end{example}
...
\begin{frame}{Tautologie notevoli: terzo escluso}
    ...
    \end{theorem}
    \begin{example}[in italiano]
        ``\textit{Carla è laureata oppure non è laureata}'' è sicuramente vera, anche se non sappiamo chi è Carla.
    \end{example}
...
\begin{frame}{Alcuni esempi in linguaggio naturale (1)}
    ...
    \begin{itemize}
        ...
        \item $A \wedge \bot \equiv \bot$: ``Michele è alto e non è alto'' è sicuramente falso. Quindi ``Carlo è alto e Michele è alto e non è alto'' $\equiv$ ``Carlo è alto''.
    \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Leggi di De Morgan (2)}
    ...
    \end{minted}
        Le spese aggiuntive si pagane se la spedizione e all'estero, oppure in Sardegna.
    \end{example}
\end{frame}
...
\begin{frame}{Il principio di dualità}
    ...
    \begin{example}
        ...
        \[A \wedge \top \equiv A\]
        Applicando ad ambo i membri dell'equivalenza gli scambi di prima, otteniamo \pause \[A \vee \bot \equiv A\]
    \end{example}
\end{frame}
...
\begin{frame}{Proprietà associativa e parentesi}
    ...
    \item Le due possibile interpretazioni sono però equivalenti, per la proprietà associativa, quindi l'ambiguità non ci crea nessun problema!
    \item La stessa cosa vale anche per $\vee$, ma non se c'è una mescolanza di $\wedge$ e $\vee$:
    \begin{itemize}
...
\begin{frame}{Equivalenze con l'or esclusivo}
    ...
    \]
    Notare che le due fp $A \wedge \neg B$ e $\neg A \wedge B$ corrispondono esattamente ai due casi in cui l'or esclusivo è vero (una delle due proposizioni è vera, l'altra è falsa).
...
\begin{frame}{L'implicazione (2)}
    ...
    \]
    \begin{example}
        Data la proposizione ``se $x$ è divisibile per 4 allora x è pari'',  vera per tutti gli $x$:
        \begin{itemize}
...
\begin{frame}{La doppia implicazione (2)}
    La doppia implicazione è anche equivalente ad una formula che usa solo and, or e not:
    \[
...
\begin{frame}{Applicare una equivalenza ad una formula (1)}
    ...
    \begin{theorem}
        Sia $X$, $X'$ ed $Y$ forme proposizionali, con $X \equiv X'$. Se $Y'$ è ottenuta da $Y$ rimpiazzando tute le occorrenze di $X$ con $X'$ o viceversa, allora $Y \equiv Y'$.
    \end{theorem}
\end{frame}
...
\begin{frame}{Applicare una equivalenza ad una formula (3)}
    ...
    \smallskip
    \begin{quotation}
        \noindent in una forma proposizionale, ogni volta che c'è un connettivo ``and'' è possibile cambiare l'ordine dei congiunti e ottenere una forma equivalente.
    \end{quotation}
...
\begin{frame}{Verificare una equivalenza logica (2)}
    ...
    \end{itemize}
    \pause
    \textbf{Questo vale anche per l'applicazione delle altre equivalenze notevoli, finché con la pratica il loro utilizzo diventa automatico.}
\end{frame}
...
\begin{frame}{Semplificazione di una fp}
    Vogliamo semplificare la forma proposizionale $X$. La tecnica è la stessa: si parte da $X$ e si applicano le equivalenze note fino ad ottenere una forma più semplice, ricordando che talvolta, per semplificare le cose, occorre prima complicarle !
