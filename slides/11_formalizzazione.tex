...
\begin{frame}{Formalizzazione}
    ...
    \medskip Non introdurremo nulla di nuovo, ma il livello di formalità molto elevato potrebbe mettervi in difficoltà. Se vi sembra che tutto ciò sia inutile, tenete conto di due cose:
    \begin{itemize}
        ...
        \item la formalizzazione è necessaria per poter scrivere programmi che manipolano formule logiche, per esempio un programma che prende in input una forma proposizionale e stabilisce se è una tautologia; grazie alla formalizzazione che vedremo il problema è più approcciabile.
        % (ma comunque alcune delle conoscenze di programmazione richieste verranno introdotte solo nel corso di ``Programmazione e Algoritmi 2'').
    \end{itemize}
\end{frame}
...
\begin{frame}{Segnatura}
    ...
    \pause
    Implicitamente, noi abbiamo sempre usato la segnatura $\Sigma$ costituita dall'insieme delle lettere maiuscole.
\end{frame}
...
\begin{frame}{Forma proposizionale (2)}
    ...
    \begin{block}<+->{Osservazione}
        ...
        \medskip
        Ad esempio, consideriamo la stringa $(A \land \land B)$. Dovrebbe essere abbastanza chiaro intuitivamente che seguendo le regole non possiamo mai mettere due $\land$ affiancati, ma la dimostrazione formale è un po' più complicata.
    \end{block}
\end{frame}
...
\begin{frame}{Interpretazione (5)}
    ...
    \begin{example}
        Se $\mcI(A)=V$, $\mcI(B)=F$ e $\mcI(C)=V$, allora $\mcI(A \to (B \lor C))= V$. Infatti:
        \begin{itemize}
            \item per calcolare  $\mcI(A \to (B \lor C))= V$ bisogna determinare $\mcI(A)$ e $\mcI(B \lor C)$
                  \begin{itemize}
                      \item $\mcI(A) = V$ per come abbiamo fissato l'interpretazione $\mcI$
                      \item \smallskip per determinare $\mcI(B \lor C)$ bisogna determinare $\mcI(B)$ e $\mcI(C)$
                            \begin{itemize}
                                \item $\mcI(B)=F$ per come abbiamo fissato l'interpretazione $\mcI$
                                \item $\mcI(C)=V$ per come abbiamo fissato l'interpretazione $\mcI$
                            \end{itemize}
                            pertanto $\mcI(B \lor C)=V$
                  \end{itemize}
                  pertanto $\mcI(A \to (B \lor C))=V$
        \end{itemize}
    \end{example}
...
\begin{frame}{Modello (1)}
    ...
    \begin{example}
        Se $\mcI(A)=V$, $\mcI(B)=F$ e $\mcI(C)=V$, allora $\mcI \models A \to (B \lor C)$ e $\mcI \not\models (A \land C) \to B$
    \end{example}
\end{frame}
...
\begin{frame}{Modello (2)}
    ...
    \begin{definition}[Modello --- definizione diretta]
        ...
        \begin{itemize}
            ...
            \item $\mcI \models X \land Y$ se e solo se $\mcI \models X$ e $\mcI \models Y$;
            \item $\mcI \models X \lor Y$ se e solo se $\mcI \models X$ o $\mcI \models Y$;
...
\begin{frame}{Modello di un insieme di formule}
    ...
    \begin{example}[Modello di un insieme di formule]
        ...
        \[
            \mcI \models \{A \to (B \lor C), \neg B\}
        \]
        ma
        \[
            \mcI \not\models \{A \to (B \lor C), \neg A\}
        \]
        perché $\mcI \not\models \neg A$.
    \end{example}
\end{frame}
...
\begin{frame}{Formula ben formata (2)}
    \begin{example}
        ...
        \begin{itemize}
            ...
            \item $(Pa \land \land Qa)$, per la presenza di due $\land$ affiancati.
        \end{itemize}
    \end{example}
    \begin{block}{Osservazioni}
        Come già visto, informalmente possiamo eliminare alcune parentesi confidando sulla priorità tra gli operatori.

        %Nel seguito quando daremo una segnatura $\Sigma$ forniremo solo le costanti individuali e predicative. Per le variabili assumeremo sempre che contengano $x$, $y$, $z$ e tutte le possibili varianti con apici e pedici.
    \end{block}
\end{frame}
...
\begin{frame}{Formalizzazione dei predicati (1)}
    ...
    \medskip
    La domanda che ci poniamo è quindi: qual è la controparte formale di un predicato espresso in linguaggio naturale?
\end{frame}
...
\begin{frame}[fragile]{Formalizzazione dei predicati (3)}
    ...
    \[
        \{ \text{(Abruzzo, Molise), (Molise, Abruzzo), (Abruzzo, Lazio),  \ldots } \} \subseteq R \times R
    \]
\end{frame}

\begin{frame}{Formalizzazione dei predicati (3)}
    In generale, un predicato di arità $n$ sul dominio $D$ può essere pensato come un sottoinsieme di $D \times D \times \cdots \times D = D^n$, ovvero l'insieme delle $n$-uple ordinate di elementi di $D$ che soddisfano il predicato.
...
\begin{frame}{Interpretazione (1)}
    Indichiamo con  $\partof(D^n)$ l'\emph{insieme delle parti} di $D^n$, ovvero l'insieme di tutti i possibili sottinsiemi di $D^n$, ovvero l'insieme di tutte le relazioni $n$-arie su $D$.
    \medskip
...
\begin{frame}{Interpretazione (2)}
    ...
    \begin{block}{Osservazione}
        ...
        \medskip
        Se consideriamo l'interpretazione dell'esempio qui sopra, in termini informali possiamo per ora affermare che $Pa$ è vero in questa interpretazione, perché $a$ viene interpretato come $0$ e $0$ è pari. Ma non possiamo dire nulla di $Px$, perché $x$ è una variabile e non sappiamo che valore attribuirgli.
    \end{block}
\end{frame}
...
\begin{frame}{Valutazione (1)}
    ...
    \begin{definition}[Valutazione]
        Dato un insieme $D$, chiamiamo \alert{valutazione su $D$} (o anche \alert{assegnamento}) una funzione $\alpha : \mc V \to D$ che associa ad ogni variabile un elemento dell'insieme $D$.
    \end{definition}

    \pause
    \begin{example}
        La funzione $\alpha(\upsilon) = 0$ che assegna il valore $0$ ad ogni variabile è una valutazione sui naturali. Notare che usiamo $\upsilon$ per riferirci a una variabile generica, mentre $v$ indica una variabile specifica in $\mc V$.
    \end{example}
...
\begin{frame}<1-2>[label=model]{Modello}
    ...
    \begin{definition}[Modello]
        Data una interpretazione $\mcI = (D, \mcI_C, \mcI_P)$ su $\Sigma$ e una valutazione $\alpha$, definiamo cosa vuol dire che $(\mcI, \alpha)$ è un modello della \fbf $X$, che scriveremo $\mcI \models_\alpha X$,  per induzione sulla struttura di $X$. In particolare:
        \begin{itemize}[<+->]
            ...
            \item \ldots analogamente per gli altri connettivi \ldots
            \item $\mcI \models_\alpha \exists \upsilon X$ se e solo se \only<.|handout:0>{???}\uncover<+->{esiste un $d \in D$ tale che $\mcI\models_{\alpha[\upsilon \mapsto d]} X$;}
...
\begin{frame}{Esempio: $\mcI \models_\alpha \exists x (Px \lor Qb)$}
    Tenendo conto della interpretazione, la formula in italiano diventa
    \begin{center}
        ``esiste un $x$ tale che $x$ è pari oppure $b$ è compreso tra $0$ e $3$''
    \end{center}

    \begin{itemize}[<+->]
        \item $\mcI \models_\alpha \exists x (Px \lor Qb)$ \only<.-12|handout:0>{?}\uncover<13->{\checkmark}
              \begin{itemize}
                  \item Dovremmo provare as assegnare ad $x$ tutti i possibili numeri naturali, fino a trovarne uno che va bene. Ma cerchiamo di usare un po' l'intuizione. La formula $Qb$ è sempre falsa, indipendentemente dalla valutazione, quindi per rendere vera $Px \lor Qb$ dobbiamo rendere vera $Px$, ovvero scegliere un valore pari per $x$.
                  \item con $x \mapsto 2$ otteniamo $\mcI \models_{\alpha[x \mapsto 2]} Px \lor Qb$ \only<.-10|handout:0>{?}\uncover<11->{\checkmark}
                        \begin{itemize}
        ...
        \item $\mcI \models_\alpha \forall x \exists y Ryx$ ?
              ...
              \begin{itemize}
                  ...
                  \item<13-> per $x \mapsto 1$: $\mcI \models_{\alpha[x \mapsto 1]} \exists y Ryx$ \only<.-15|handout:0>{?}\uncover<16->{\checkmark}
                        ...
                        \end{itemize}
                  \item<17-> in teoria dovremmo ripetere il discorso per ogni numero naturale. Invece, facciamo una \emph{dimostrazione} prendendo un $d$ generico.
              \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}{Esempio: $\mcI \not\models_\alpha \forall x \exists y Rxy$}
    ...
    \begin{center}
        ``per ogni $x$ esiste un $y$ tale che $x$ è maggiore di $y$''
    \end{center}
    o anche
    \begin{center}
...
\begin{frame}{Esempio: $\mcI \models_\alpha \forall x (\neg P x \to \exists y Rxy)$ (1) }
    ...
    \]
    è simile a quella precedente, ma ora la premessa $\neg Px$ limita il quantificatore $\forall x$ ai soli valori di $x$ che non soddisfano $P$.
                  ...
                  \item<8-> per $x \mapsto 1$ otteniamo $\mcI \models_{\alpha[x \mapsto 1]} \neg P x \to \exists y Rxy$ \only<.-14|handout:0>{?}\uncover<15->{\checkmark}
                        \begin{itemize}
                            ...
                            \item<9-> $\mcI \models_{\alpha[x \mapsto 1]} \neg P x$ \only<.-10|handout:0>{?}\uncover<11->{\checkmark}
                                  ...
                                  \uncover<10->{
                                      \smallskip
                                      \qquad $\mcI \models_{\alpha[x \mapsto 1]} P x$ \only<.|handout:0>{?}\uncover<+->{\xmark}
                                  }
                            \item<12-> $\mcI \models_{\alpha[x \mapsto 1]} \exists y Rxy$ \only<.-13|handout:0>{?}\uncover<14->{\checkmark \quad scegliendo $y \mapsto 0$}
                        \end{itemize}
                  \item<16-> per gli altri valori di $x$, tutto procede come il caso $x \mapsto 0$ o $x \mapsto 1$ a seconda che il numero sia pari o dispari.
              \end{itemize}
...
\begin{frame}{Valutazioni e variabili libere}
    ...
    \end{theorem}
    \begin{block}{Osservazione}
        Notare che ci interessa il valore della valutazione solo per le variabili \textbf{libere}. Per determinare se $\mcI \models_\alpha \forall x Px$, il valore di $\alpha(x)$ non ci interessa, tanto verrà sostituito da un valore $d$ nel dominio $D$.
    \end{block}
    \begin{block}{Osservazione}
        In particolare, il valore di verità delle formule chiuse non dipende dalla valutazione!
    \end{block}
\end{frame}
...
\begin{frame}{Altre definizioni}
    ...
    \begin{definition}[Tautologia e contraddizione]
        Una \fbf $X$ è detta \emph{valida} se e solo se $\mcI \models_\alpha X$ per tutte le interpretazioni $\mcI$ e valutazioni $\alpha$. Una \fbf $X$ è detta \emph{contraddizione} se e solo se $\mcI \not\models_\alpha X$ per tutte le interpretazioni $\mcI$ e valutazioni $\alpha$.
    \end{definition}
